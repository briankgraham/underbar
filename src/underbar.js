(function() {
  ...
  _.each = function(collection, iterator) {
    if (Array.isArray(collection)){
      ...
      for (var x in collection){ iterator(collection[x], x, collection); }
    }
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    _.each(collection, function(item){
      typeof accumulator === 'undefined' ? accumulator = item : accumulator = iterator(accumulator, item);
    });
    return accumulator;
  };
  ...
  _.some = function(collection, iterator) {
    ...
    /** With a loop (faster than with Every, stops when it reaches a true statement */
  ...
  _.extend = function(obj) {
    _.each(arguments, function(item){
      _.each(item, function(value, key){ // remember _.each takes value, key, collection (order)
        obj[key] = value;
      });
    });
    return obj;
  ...
  _.flatten = function(nestedArray, result) {
    result = result || [];
    return _.reduce(nestedArray, function(current, next){
      return current.concat(Array.isArray(next) ? _.flatten(next, result) : next);
    }, result);
  };
